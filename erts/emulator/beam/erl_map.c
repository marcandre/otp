...
BIF_RETTYPE maps_putx_3(BIF_ALIST_3) {
    if (is_map(BIF_ARG_3)) {
	BIF_RET(erts_maps_putx(BIF_P, BIF_ARG_1, BIF_ARG_2, BIF_ARG_3));
    }
    BIF_P->fvalue = BIF_ARG_3;
    BIF_ERROR(BIF_P, BADMAP);
}
...
static int is_potentially_expensive_to_order(Eterm key) {
    return 0;
}
...
Eterm erts_maps_putx(Process *p, Eterm key, Eterm value, Eterm map) {
	...
	*hp++ = mp->keys;
	...
	} else if (is_potentially_expensive_to_order(key)) {
	    for( i = 0; i < n; i ++) {
		if (EQ(ks[i], key)) {
		    goto found_key;
		} else {
